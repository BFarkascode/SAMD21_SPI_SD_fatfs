...
Another annoying thing that I have learnt through trial and error was that the bus isn't "empty" when the master isn't asking for data. Instead, the bus is constantly full of trash, only going to "idle" (dataline 0xFF) after certain scenarios are met. This means that we MUST ALWAYS fish for the tokens on the bus, we can't just start taking data immediately after we have demanded it from the card.
