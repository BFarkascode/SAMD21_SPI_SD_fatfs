...
Admittedly, one of the most difficult things for me had been to make hands-or-tails on how nthe SD card is being controlled. You see, all SD cards come with two distinct control systems between which you can choose upon power up by pulling the card's CS pin HIGH or LOW. For microcontrollers, only the CS pin LOW version will work, which will emulate the SD card control using the SPI bus. But, despite using the SPI bus as means of communicating, the SD card driver IS NOT an SPI-style driver! Instead of having responses coming in immediatelly as bytes reach the slave, the communicating will work along "tokens", "replies" and "commands".

Tokens will be specific bytes depending on what the card is doing or expected to do and they preceed the data package that is being transferred on the bus. These tokens come on the bus sometime and somewhere, but NOT immiediatelly when they are demanded (think of it as communicating with a very busy other microcontroller that will reply whenever it can, but not immediatelly since it is also doing something). Tokens can indicate the start or the end of a data package, as well as show that the data sent to the card has not been accepted (i.e. there was a transmission error). There are 5 different tokes (see SD card datasheet 7.3.3 Control tokens section).

Replies come in the form of standard answer formats as well which are then have to be disected in order to acquire the relevant information. Apart from receiving a full 512 byte data package as it is when we ask for the CSD and CIS values, we can have R1, R2, R3 and R7 as a response, each representing 1/2/3 or 7 bytes.
...
One nasty thing is that initialization of the card must always occur at 400 kHz or less, despite the fact that the SPI might be set for 4 MHz or even faster. No, within the driver, we MUST slow down the SPI bus during card initialization, then speed it back up afterwards.

An annoying thing that I have learnt through trial and error was that the bus isn't "empty" when the master isn't asking for data. Instead, the bus is constantly full of trash, only going to "idle" (dataline 0xFF) after certain scenarios are met. This means that we MUST ALWAYS fish for the tokens on the bus, we can't just start taking data immediately after we have demanded it from the card.

Another annoying thing is the timing of the card. We need to know exactly, what type of answer we are expecting and then send over enough dummies by the master to properly vacate the SD card. We can't just cut the communication short. Similarly, it is recommended by the datasheet to send a dummy byte over after certain commands are sent (see 4.4 Clock control section).

Lastly, we reserve the driver for only SDHC card types, which are up to 32 GB big. This means that we will always manipulate the memory area in blocks of 512 bytes, never less.

To give a quick rundown on how we control the SD card:
1) We need to disable the card by pullling CS HIGH and then send over 10 bytes of dummy data. This is recommended by the datasheet.
...
4) We publish CMD55 (to select ACMD type commands) and ACMD41 until we receive in the R1 reply of these commands that the card is ready. For this layer and on, we don't care about the CRC (unless we end up activating it)

Afterwards, we can call additional commands to extract card ID or to validate the card selection, but these are not crucial. Once the card is ready, it will stay as such until it is power cycled.

Of note, the argument for all write and read commands (so, CMD17, CMD18, CMD24 and CMD25) is the sector address, not the byte address. If we provide the argument 0x8000, then 0x8001 will be the next value to update. It just must be kept in mind that sector addresses range from 0x0 to 0xF01D (0xF01d * 512 is 32 GB) on a 32 GB card. Tokens are inserted in a rather self-explanatory way.
